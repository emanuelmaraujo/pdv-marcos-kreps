...
async function autoConfirmOnlinePaidOrder(supabaseAdmin: any, orderId: string) {
  const { data: order } = await supabaseAdmin
    .from("orders")
    .select("id, daily_number, status, type, source, customer_name, customer_phone, notes, discount_amount, packing_fee, total_amount, payment_status, payment_method, branch_id, branches(printer_config)")
    .eq("id", orderId)
    .single();
