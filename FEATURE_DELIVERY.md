...
### Mismatch de hidratação do React (investigado e corrigido)
Ao tentar testar no navegador pela primeira vez, nenhuma página do app aceitava clique/digitação em `next dev` — reproduzido também em `/login`, página não tocada originalmente por esta mudança. Investigação em duas partes:

1. `src/app/layout.tsx` colocava um `<script dangerouslySetInnerHTML>` bruto manualmente dentro de `<head>` (bootstrap de tema, pra evitar flash do tema errado). Trocado pelo componente `next/script` com `strategy="beforeInteractive"`, dentro de `<body>` — padrão documentado do Next.js para este caso.
2. **Causa raiz de verdade, confirmada por teste de controle** (build de produção, `data-theme` sozinho não bastava): `src/app/login/page.tsx` calculava `showBiometric` com `useState(() => canTryDiscoverablePasskey())` — essa função lê `window`/`navigator.credentials`, então no servidor (sem `window`) sempre retornava `false`, mas no cliente (browsers com suporte a WebAuthn) retornava `true` — o botão de passkey aparecia/sumia entre o HTML do servidor e a primeira renderização do cliente, causando o mismatch (React error #418). Confirmado forçando o valor pra uma constante e vendo o erro desaparecer por completo; revertido e depois confirmado de novo que ele reaparecia com o código original.
   - Corrigido com `useSyncExternalStore(() => () => {}, canTryDiscoverablePasskey, () => false)` — `getServerSnapshot` sempre `false` (igual ao servidor), `getSnapshot` roda a checagem real só no cliente depois do mount. Evita o mismatch sem usar `useEffect` + `setState` (que a regra `react-hooks/set-state-in-effect` deste repo rejeita).
- Confirmado: em **modo dev** (`next dev`/Turbopack) esse erro de hidratação travava a interatividade da página inteira; em produção (`next start`) o React conseguia se recuperar sozinho, por isso o bug só apareceu como "nada funciona" no ambiente de preview usado para testar visualmente.
- Validado depois da correção: zero erros no console em `/`, `/login` e `/pedir/principal`, tanto em `next start` quanto em `next dev`; clique e digitação reais (via `computer` tool, não só JS) funcionando em `/login`.
- Não foi feita uma varredura completa do resto do app atrás do mesmo padrão (`useState(() => algumaChecagemDeNavegador())`) — só as páginas tocadas neste teste (`/`, `/login`, `/pedir`) foram confirmadas limpas.
